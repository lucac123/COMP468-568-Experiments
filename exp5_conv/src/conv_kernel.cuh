...
__global__ void conv2d_naive_kernel(const float *__restrict__ input,
  ...
  float acc = 0.0f;
  /* TODO(student): loop over channels/ksize and accumulate into acc. Remember
...
__global__ void conv2d_tiled_kernel(const float *__restrict__ input,
  ...
  float *tile_weight =
      tile + BLOCK_SIZE * BLOCK_SIZE; // placeholder layout suggestion
  /* TODO(student): stage input patches / filter tiles into shared memory and
     reuse them across threads within the block. Carefully handle halos caused
     by padding. */
  (void)tile_input;
  (void)tile_weight;
  (void)output;
...
inline void launch_naive_conv2d(const float *d_input, const float *d_weight,
  ...
  conv2d_naive_kernel<<<grid, block, 0, stream>>>(d_input, d_weight, d_output,
                                                  shape);
  /* TODO(student): check cudaGetLastError() and optionally
...
inline void launch_tiled_conv2d(const float *d_input, const float *d_weight,
  ...
  conv2d_tiled_kernel<<<grid, block, shared_bytes, stream>>>(d_input, d_weight,
                                                             d_output, shape);
  /* TODO(student): choose a better shared-memory layout/size expression once
